...
#
# =============================================================================
#  Complete Flutter (Android) development environment setup for macOS.
# =============================================================================
#
# SYNOPSIS
#   Unattended installer that takes a fresh macOS machine (Apple Silicon or
#   Intel) to a fully working Flutter Android toolchain:
#
#     1. Prerequisites            (git / curl / unzip / tar / shasum — detected,
#                                  never installed: that needs a GUI prompt)
...
#   sudo, no Homebrew, no Android Studio, no Xcode toolchain. The script is
...
#   Deliberate macOS differences are marked "macOS:" in the comments.
#   bash 3.2 compatible on purpose — that is what macOS still ships as
#   /bin/bash: no associative arrays, no mapfile, no ${var,,}, no namerefs.
# =============================================================================
...
CFG_FLUTTER_RELEASES_JSON='https://storage.googleapis.com/flutter_infra_release/releases/releases_macos.json'
...
# Pinned Flutter fallbacks — stable 3.44.8, both architectures are published
CFG_FLUTTER_FALLBACK_VERSION='3.44.8'
CFG_FLUTTER_FALLBACK_URL_ARM64='https://storage.googleapis.com/flutter_infra_release/releases/stable/macos/flutter_macos_arm64_3.44.8-stable.zip'
CFG_FLUTTER_FALLBACK_SHA_ARM64='c3d6fe95078f7001d947a31d42527de91d5bfe62e4cf444a1493a2e8f1fb199d'
CFG_FLUTTER_FALLBACK_URL_X64='https://storage.googleapis.com/flutter_infra_release/releases/stable/macos/flutter_macos_3.44.8-stable.zip'
CFG_FLUTTER_FALLBACK_SHA_X64='b2f765234217327a5859d046c9f3b167387b61da5408b5866ed448d905877c66'

# Pinned Temurin fallbacks — jdk-17.0.20+8.
# macOS: the mac archive is a .tar.gz (Windows ships a .zip) and it uses the
# Apple bundle layout, so java lives in <dir>/Contents/Home/bin/java.
CFG_JDK_FALLBACK_URL_AARCH64='https://github.com/adoptium/temurin17-binaries/releases/download/jdk-17.0.20%2B8/OpenJDK17U-jdk_aarch64_mac_hotspot_17.0.20_8.tar.gz'
CFG_JDK_FALLBACK_SHA_AARCH64='524850138c742324fb21fca4ff6ef68ea25f25bf59366a864e45b4a0c45ed0df'
CFG_JDK_FALLBACK_URL_X64='https://github.com/adoptium/temurin17-binaries/releases/download/jdk-17.0.20%2B8/OpenJDK17U-jdk_x64_mac_hotspot_17.0.20_8.tar.gz'
CFG_JDK_FALLBACK_SHA_X64='3710c3131c5d7c090582b357f1310133a90bf701183d065223f1a0b90b9ed5ae'

# Pinned Android cmdline-tools fallbacks — revision 22.0, build 15859902.
# macOS: unlike Windows there is NO single "commandlinetools-mac-<build>.zip"
# any more; the mac builds are architecture specific and the generic name 404s.
# Google publishes only a SHA-1 for these archives (repository2-3.xml carries
# <checksum type="sha1">), so this is the one download verified with shasum -a 1
# instead of -a 256. Flutter and the JDK do publish SHA-256 and use it.
CFG_CMDLINE_FALLBACK_URL_ARM64="$CFG_ANDROID_REPO_BASE/commandlinetools-mac_arm64-15859902_latest.zip"
CFG_CMDLINE_FALLBACK_SHA1_ARM64='c4e0dbc53f1a4ce04fc2b11d4e2c4675001a95af'
CFG_CMDLINE_FALLBACK_URL_X64="$CFG_ANDROID_REPO_BASE/commandlinetools-mac_x86_64-15859902_latest.zip"
CFG_CMDLINE_FALLBACK_SHA1_X64='1a79e61677b99cb0ce622792713cbea7ff2dc5de'
...
ARCH_FLUTTER=''      # arm64 | x64        (releases_macos.json dart_sdk_arch)
ARCH_ADOPTIUM=''     # aarch64 | x64      (Adoptium API architecture)
ARCH_CMDLINE=''      # mac_arm64 | mac_x86_64  (filename token in repository2-3.xml)
...
usage() {
    cat <<'USAGE'
Flutter (Android) development environment setup for macOS.
...
What it changes (also the uninstall checklist):
  ...
  * one marker-delimited block in your shell profile (~/.zprofile for zsh)
    exporting ANDROID_HOME, JAVA_HOME and PATH
...
No sudo, no Homebrew, nothing outside your home directory.
USAGE
}
...
human_mb() {
    # Portable file size in MB (stat -f is BSD/macOS)
    local bytes
...
# ---- Minimal JSON reader ---------------------------------------------------
# ---------------------------------------------------------------------------
# No jq: the contract forbids new dependencies and a stock macOS has no jq.
...
resolve_flutter() {
    ...
    RESOLVED_ALGO='256'
    if [ "$ARCH_FLUTTER" = 'arm64' ]; then
        RESOLVED_URL="$CFG_FLUTTER_FALLBACK_URL_ARM64"
        RESOLVED_SHA="$CFG_FLUTTER_FALLBACK_SHA_ARM64"
    else
...
resolve_jdk() {
    ...
    RESOLVED_ALGO='256'
    if [ "$ARCH_ADOPTIUM" = 'aarch64' ]; then
        RESOLVED_URL="$CFG_JDK_FALLBACK_URL_AARCH64"
        RESOLVED_SHA="$CFG_JDK_FALLBACK_SHA_AARCH64"
    else
    ...
    api="$CFG_TEMURIN_ASSETS_API?os=mac&architecture=$ARCH_ADOPTIUM&image_type=jdk&vendor=eclipse"
...
resolve_cmdline_tools() {
    ...
    RESOLVED_ALGO='1'
    if [ "$ARCH_CMDLINE" = 'mac_arm64' ]; then
        RESOLVED_URL="$CFG_CMDLINE_FALLBACK_URL_ARM64"
        RESOLVED_SHA="$CFG_CMDLINE_FALLBACK_SHA1_ARM64"
    else
    ...
    # carries our architecture token. The two macOS archives share
    # host-os=macosx, so the filename is what distinguishes them.
    ...
    ' <<< "$raw")
    if [ -z "$line" ]; then
        write_warn 'No macOS cmdline-tools entry found in the manifest; using the pinned URL'
        return 0
    fi
...
profile_block_text() {
    ...
    printf '%s\n' "$MARK_BEGIN"
    printf '%s\n' '# Written by flutter-dev-setup (macos/setup.sh). Re-run the script to update;'
...
write_profile_block() {
    ...
    if [ -f "$PROFILE_FILE" ]; then
        mode=$(stat -f '%Lp' "$PROFILE_FILE" 2>/dev/null || echo '')
        if [ -n "$mode" ]; then
...
set_jvm_network_defaults() {
    ...
    # macOS: unlike the Windows script this does NOT override the truststore
    # (-Djavax.net.ssl.trustStoreType=KeychainStore) — on macOS that swaps the
    # JDK's own working cacerts for the login keychain and can break TLS that
    # was fine, so it is left alone deliberately.
...
# ---------------------------------------------------------------------------
detect_arch() {
    local machine translated arm_capable
    machine=$(uname -m)
    translated=$(sysctl -n sysctl.proc_translated 2>/dev/null || echo 0)
    arm_capable=$(sysctl -n hw.optional.arm64 2>/dev/null || echo 0)
    if [ "$machine" = 'x86_64' ] && { [ "$translated" = '1' ] || [ "$arm_capable" = '1' ]; }; then
        write_warn 'This shell is running under Rosetta on an Apple Silicon Mac;'
        write_warn 'installing the native arm64 toolchain anyway.'
        machine='arm64'
    fi
    case "$machine" in
        arm64)
            ARCH_FLUTTER='arm64'; ARCH_ADOPTIUM='aarch64'; ARCH_CMDLINE='mac_arm64' ;;
        x86_64)
            ARCH_FLUTTER='x64';   ARCH_ADOPTIUM='x64';     ARCH_CMDLINE='mac_x86_64' ;;
        *)
            die "Unsupported architecture: $machine (expected arm64 or x86_64)" ;;
    esac
    write_ok "Architecture: $machine (Flutter $ARCH_FLUTTER, Temurin $ARCH_ADOPTIUM, cmdline-tools $ARCH_CMDLINE)"
}

check_required_tools() {
    # Detect-or-instruct, never hang: these all ship with macOS or the Xcode
    # Command Line Tools, and the CLT cannot be installed unattended.
    local missing='' t
    for t in curl unzip tar shasum awk sed mkfifo; do
        if ! have_cmd "$t"; then
            missing="$missing $t"
        fi
    done
    if [ -n "$missing" ]; then
        die "Required tool(s) not found:$missing" \
            'These ship with macOS and the Xcode Command Line Tools. Install them with:' \
            '' \
            '    xcode-select --install' \
            '' \
            'then re-run this script.'
    fi
    write_ok 'Required tools present: curl, unzip, tar, shasum'
}

resolve_git() {
    # macOS: /usr/bin/git is a stub that opens a GUI installer dialog when the
    # Command Line Tools are absent, so `command -v git` proves nothing. Look
    # for a real git binary instead and never execute the stub.
    local dev candidate
    GIT_BIN=''
    dev=$(xcode-select -p 2>/dev/null || true)
    if [ -n "$dev" ] && [ -x "$dev/usr/bin/git" ]; then
        GIT_BIN="$dev/usr/bin/git"
        return 0
    fi
    ...
    if [ -n "$candidate" ] && [ "$candidate" != '/usr/bin/git' ] && [ -x "$candidate" ]; then
        GIT_BIN="$candidate"
        return 0
    fi
    return 1
...
preflight() {
    ...
    if [ "$major" -lt "$CFG_MIN_MACOS_MAJOR" ]; then
        die "macOS $CFG_MIN_MACOS_MAJOR or newer is required (found $product)."
    fi
    write_ok "macOS version: $product"
...
# ---------------------------------------------------------------------------
install_git() {
    write_step 'Git (Xcode Command Line Tools)'
    ...
    write_fail 'Git was not found and cannot be installed unattended on macOS.'
    add_summary 'Git' 'MISSING' 'run: xcode-select --install'
    if [ "$VERIFY_ONLY" -eq 1 ]; then
        # Read-only report: record the row and keep going.
        return 0
    fi
    die 'Git is required by the Flutter SDK and only ships with the Xcode Command Line Tools.' \
        'Installing them opens a GUI dialog, so this script will not do it for you. Run:' \
        '' \
        '    xcode-select --install' \
        '' \
...
# ---------------------------------------------------------------------------
jdk_home_in() {
    # macOS: Temurin uses the Apple bundle layout, so java is at
    # <dir>/Contents/Home/bin/java. Detect it rather than hardcoding, and keep
    # the plain <dir>/bin/java case so a layout change does not break this.
...
install_android_packages() {
    ...
    if [ "$VERIFY_ONLY" -eq 1 ]; then
        # Note: a Homebrew adb on PATH proves nothing, so only our own paths
        # are checked here.
...
run_flutter_doctor() {
    ...
    # may count as failures. [X] Xcode / Chrome / Android Studio are out of
    # scope (see Non-goals in CONTRIBUTING.md).
    ...
    if [ -n "$scoped" ]; then
        ...
        add_summary 'flutter doctor' 'Passed (unrelated issues)' \
            'out-of-scope items flagged (e.g. Xcode/Chrome/Android Studio)'
    else
